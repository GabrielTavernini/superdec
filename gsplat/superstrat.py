...
class SuperStrategy(Strategy):
    ...
    def check_sanity(
        ...
        Check if:
            ...
            * The following keys are present: {"scales", "quats", "opacities"}.
        ...
        for key in ["scales", "quats", "opacities"]:
    ...
    def step_post_backward(
        ...
        if (
            ...
            # prune GSs
            if self.prune_sq or self.prune_bg:
    ...
    def _grow_gs(
        ...
        is_split = torch.cat(
            [
                is_split[:Ng],
                torch.zeros(is_dupli[:Ng].sum().item(), dtype=torch.bool, device=device),
                is_split[Ng:],
                torch.zeros(is_dupli[Ng:].sum().item(), dtype=torch.bool, device=device),
            ]
        )
    ...
    def _prune_gs(
        ...
        is_prune[:Ng] &= self.prune_sq
        is_prune[Ng:] &= self.prune_bg
