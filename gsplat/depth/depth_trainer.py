...
import viser
import yaml
from depth_colmap import Dataset, Parser
...
from depth_superq import SuperQ, SuperQConfig
from depth_superstrat import SuperStrategy
...
class Config:
    ...
    show_superqs = True
    # Superq background
    num_pts_background: int = 150000 # -1 will use all pointcloud points
...
class Runner:
    ...
    def __init__(
        ...
        self.trainset = Dataset(
            ...
            ply_path=f"data/scannetpp/data/{cfg.scene_id}/scans/mesh_aligned_0.05.ply",
        )
        ...
        Nsqg = len(self.splats["opacities"])
        self.render_quats = torch.full((Nsqg, 4), 0.0, device=self.device)
        self.render_quats[:, 0] = 1.0
        self.render_scales = torch.full((Nsqg, 1), self.cfg.init_scale, dtype=torch.float32, device=self.device)
        self.render_scales = self.render_scales.repeat(1, 3)
        self.render_opacities = torch.full((Nsqg,), self.cfg.init_opa, device=self.device)
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        render_quats = self.render_quats
        render_scales = self.render_scales
        render_opacities =  self.render_opacities
