...
def visualize_handler(server, superq, sdf_values, normals, plot = False):
    ...
    if hasattr(superq, 'normals'):
        p_normals = superq.normals.detach().cpu().numpy()
        p1 = points
        p2 = points + (p_normals * 0.01)
        line_segments = np.stack([p1, p2], axis=1)
        ...
        similarity = np.sum(p_normals * normals, axis=1)
        cmap = plt.get_cmap('RdBu')
        norm = plt.Normalize(vmin=-1, vmax=1)
        normal_colors = cmap(norm(similarity))
        normal_colors = normal_colors[:, :3]
        server.scene.add_point_cloud(
            name="/normal_pointcloud",
            points=points,
...
def main():
    ...
    for epoch in pbar:
        ...
        sdf_values, normals = superq.forward()
        ...
        Lreg = 0.01 * torch.norm(superq.scale(), p=1, dim=1).mean()

        Lnorm = 0.05 * (1-torch.mean(torch.sum(normals * superq.normals, axis=1)))
        
        loss = Lsdf + Lreg + Lnorm
        if torch.isnan(loss):
        ...
        if epoch % 20 == 0:    
            visualize_handler(server, superq, sdf_values, normals)
        pbar.set_postfix({"Lsdf": f"{Lsdf.item():.6f}", "Lnorm": f"{Lnorm.item():.6f}", "Loss": f"{loss.item():.6f}"})
    visualize_handler(server, superq, sdf_values, normals, plot=True)
