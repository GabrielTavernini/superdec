...
class SuperQ(nn.Module):
    def __init__(
        ...
        if ply:
            ...
            for i in range(1):
                curr_length = self.line_length * (i+1)
                new_points = self.points + (self.normals * curr_length)
                distances = torch.cdist(new_points, og_points) # using all points leads to OOM
