...
class SuperQ(nn.Module):
    def __init__(
        ...
        trainable = ["raw_scale", "raw_exponents", "raw_rotation", "raw_tapering", "raw_bending", "translation"]
        ...
        self.raw_bending = torch.full((self.N, 2), 1e-4, dtype=torch.float, device=device)
        self.raw_bending[:, 0] = math.log(1e-2) # no bending
    ...
    def bending(self):
        kb = torch.exp(self.raw_bending[:, 0:1]) + 1e-6
    ...
    def update_handler(self, compute_meshes=True):
        ...
        bending_values = torch.cat([kb, alpha], dim=1)
        self.pred_handler.bending[self.idx][mask] = bending_values.detach().cpu().numpy()
    ...
    def apply_bending(self, x, y, z):
        N, M = x.shape
        kb, alpha = self.bending()  # kb: (N, 1), alpha: (N, 1)
        
        # Forward cylindrical transformation
        angle_offset = torch.atan2(y, x)  # (N, M)
        R = torch.sqrt(x**2 + y**2) * torch.cos(alpha - angle_offset)  # (N, M)        
        gamma = torch.atan2(z, (1.0 / kb) - R)  # (N, M)
        r = (1.0 / kb) - torch.sqrt(z**2 + ((1.0 / kb) - R)**2)  # (N, M)
    ...
    def sdf_batch(self, points):
        ...
        # points: (1, 3, M)
        points_expanded = points.unsqueeze(0) #(1, 3, M)
        ...
        # Apply tapering
        x, y, z = self.apply_tapering(x, y, z)  # (N, 3, M)
        
        # Aapply bending transformation
        x, y, z = self.apply_bending(x, y, z)  # (N, 3, M)
