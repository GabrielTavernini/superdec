...
def main():
    truncation = 0.05
    pred_handler = PredictionHandler.from_npz("data/output_npz/objects/round_table.npz")
    ...
    for epoch in pbar:
        ...
        if epoch % 20 == 0:    
            visualize_handler(server, superq, sdf_values)
        pbar.set_postfix({"Lsdf": f"{Lsdf.item():.6f}", "Loss": f"{loss.item():.6f}"})
    visualize_handler(server, superq, sdf_values, plot=True)
