...
class SuperQ(nn.Module):
    def __init__(
        ...
        trainable = ["raw_scale", "raw_exponents", "raw_rotation", "raw_tapering", "raw_assign_matrix", "translation"]
        ...
        self.raw_scale = torch.log(torch.clamp(raw_scale - self.minScale, min=1e-6))
        ...
        # raw_assign_matrix = torch.tensor(pred_handler.assign_matrix[self.idx].T[self.mask], dtype=torch.float, device=device).squeeze() # [N, 4096]
        # raw_assign_matrix += torch.abs(torch.randn_like(raw_assign_matrix)) * 5e-2
        # self.raw_assign_matrix = torch.log(raw_assign_matrix * 0.1 + 1)
        # self.raw_assign_matrix = -self.sdf_batch(self.points.T)
        self.raw_assign_matrix = torch.ones((self.N, self.points.shape[0]), dtype=torch.float, device=device)
        ...
        if use_full_pointcloud and 'ply_points' in locals():
            ...
            full_assign_matrix = self.raw_assign_matrix[:, nearest_indices]
            self.raw_assign_matrix = full_assign_matrix
    ...
    def scale(self):
        ...
        return torch.exp(self.raw_scale) + self.minScale
    ...
    def assign_matrix(self):
        return F.softmax(self.raw_assign_matrix, dim=0)
    ...
    def forward(self):
        ...
        ## Surface values
        weights_points = self.assign_matrix()
        softmax_std = torch.mean(torch.std(weights_points, dim=0))
        values_points = torch.sum(weights_points * all_sdfs_leaky[:, :split_idx], dim=0)
        # mins, _ = torch.min(all_sdfs_leaky[:, :split_idx], dim=0)
        # mask = mins < 0
        # values_points[mask] = mins[mask]
        
        # Calculate counts
        idx_points = torch.argmin(all_sdfs_leaky[:, :split_idx], dim=0)
        ...
        return values_points, values_outside, counts_points, counts_outside, softmax_std
